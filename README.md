...
* [Try it!](#try-it)
  ...
  * [Visualize your own data](#local-data)
...
It can display up to 3 sets of multi-modal, regularly inter-spaced, large image slices along the 3 standard orthogonal axes (Axial, Coronal, Sagittal). Each slice view may contains several raster images layers, and Atlas regions (represented by aligned vector images, SVG) displayed as an overlay over the raster images.

<div><video controls src="https://user-images.githubusercontent.com/64643044/165652526-0eff29be-0f3d-46ec-90de-1f80d716a344.mp4" style="max-width: 100%; height: auto;" /></div>
...
## Try it! <a id="try-it"></a>

There are several ways to try ZAViewer, depending on your needs and technical skills.
...
2. Launch ZAViewer by opening the following URL in your web browser :<a id="run-localcont-remotedata"></a>

    [`http://localhost:9090/?datasrc=https://dataportal.brainminds.jp/ZAViewer_BMA_2023/`](http://localhost:9090/?datasrc=https://dataportal.brainminds.jp/ZAViewer_BMA_2023/)
...
## C) Visualize your own data<a id="local-data"></a>
...
* region information, and their hierarchical organization,
...
Moreover, in case of multi-plane configuration:
...
##### 1.3.2. Image for subview widget

* In case of single axis image set, you may provide a small image from on an orthogonal plane which will be displayed in the subview widget (it allows to see the current slice position in the set)

#### 1.4. Run the script utility (Docker image)
...
* It is of course possible to prepare distinct sets of images into distinct output directories, and display one or the other by specifying the right path when running the web-server container (just change the `/full/path/to/output/dir` path).
...
* For MS-Windows, Docker might not be able to mount the output directory if it is located on a removable media.
* For MS-Windows, if you are using WSL2 backend for Docker, you might need to copy the output directory inside the WSL filesystem (e.g. under `/home/your_user/`) to be able to mount it in the container.
...
2. Add your custom processing :

    * declare your custom processing code inside the following source file : [`zaviewer/extension/ZAVProcessings.js`](https://github.com/brainminds-dataportal/zaviewer/blob/master/extension/ZAVProcessings.js)
...
3. Load ZAViewer and test your processing!
    ...
    The same ZAViewer Docker image as before is used, but with different parameters to allow running custom processing code.
...
1. Run the UI+Editor container:
    ...
    * When running in editing mode, ZAViewer works on an independent set of SVG files (basically copied and renamed version of the previously used SVG).
    ...
    * All files and directories must be readable & writeable from the Docker container. (🟠 add more details here).
...
* `NodeJs` ^v24.0.0 (and a recent `npm`) must be installed on your platform beforehand
...
### Set-up to install packages from Github registry (for **first time build**)<a id="dev-setupnpmrc"></a>
...
| `verofdata.all.uri` | URI for the link displayed with the dataset description |
| `first_access` | |
...
| `coronal_size` | _optional_ <sup>1</sup> _see above_ |
| `sagittal_size` | _optional_ <sup>1</sup> _see above_ |
...
* One path defining a closed rectangle the size of the image must be defined with `id="background"` (This element is **mandatory** for correct region rendering and region edition feature).
...
This optional resource is used by ZAViewer to dynamically show the long name of focused region, or to display hierarchical organization of all brain regions.

Regions are described in the resource by `json` blocks which follow the `typescript` interface below.
Each region is identified by a unique region abbreviation, which is specified in `bma:regionId` (or `xml:id`) attribute of overlay SVG paths' in order to make the link between shapes and brain regions.
