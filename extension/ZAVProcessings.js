  ...
  addProcessor(
    // Processor using ONNX (see https://github.com/microsoft/onnxruntime/tree/master/js/)
    {
      ...
      processImageData: (imageData) => {
        ...
        return ort.InferenceSession.create('extension/models/onnx/udnie-9.onnx', {
          ...
          .then((session) => {
            ...
            const feeds = {
              input1: new ort.Tensor('float32', modfn.imageDataPixs2FnstPixs(imageData.data), [1, 3, width, height]),
            };
  ...
  addProcessor(
    // Processor based on Pix2Pix from ml5.js - https://learn.ml5js.org/#/reference/pix2pix
    {
      ...
      processImageData: (imageData) =>
        ...
        ml5
          .pix2pix('extension/models/edges2pikachu.pict')
          .then((pix2pixModel) => {
            ...
            const nbBlocks = Math.max(
              Math.min(Math.floor(imageData.width / PixelPerBlock), Math.floor(imageData.height / PixelPerBlock)),
              1,
            );
            ...
            return {
              model: pix2pixModel,
              canvas: modfn.imageDataToCanvas(imageData, {
                width: nbBlocks * PixelPerBlock,
                height: nbBlocks * PixelPerBlock,
              }),
            };
          })
          .then((params) => {
